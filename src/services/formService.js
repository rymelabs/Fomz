...
export const createForm = async (formData, userId) => {
  try {
    ...
    await updateDoc(userRef, { "usage.forms": increment(1) });
...
export const getFormByShareId = async (shareId) => {
  try {
    ...
    if (
      ...
      console.error(
        "Missing Firestore index. Please create an index for forms: shareId ASC, settings.published ASC"
      );
    }
    ...
    if (
      ...
      const friendly = new Error(
        "Insufficient Firestore permissions. The form might not be published."
      );
...
export const getUserForms = async (userId) => {
  try {
    ...
    if (
      ...
      const friendly = new Error(
        "Insufficient Firestore permissions to retrieve forms. Please update your Firestore rules to allow reads for this user (see README or Firebase console)."
      );
      friendly.code = "permission-denied";
      throw friendly;
    }
